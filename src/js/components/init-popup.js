...
const initPopup = (btnSelector, popupSelector, byId = false) => {
	...
	const openPopup = (e, $popup) => {
		allActivePopups = document.querySelectorAll(`.${CLASSNAMES.popupActiveState}`);

		e.preventDefault();
		$popup.classList.add(CLASSNAMES.popupActiveState);
		document.body.classList.add(CLASSNAMES.bodyPopupOpenState);

		if (!exist(allActivePopups)) return;

		allActivePopups.forEach((item) => item.classList.remove(CLASSNAMES.popupActiveState));
	};
